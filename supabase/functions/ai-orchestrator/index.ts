...
if(body.action==="validate_render"){
 ...
 const fps=Number(video?.r_frame_rate?.split("/")?.[0])/Number(video?.r_frame_rate?.split("/")?.[1]||1);
 checks.push({check:"video_frame_rate_valid",passed:!Number.isFinite(fps)||fps>=23});
 const expectedResolution=String(run.brief?.resolution||"1080p");
 ...
 checks.push({check:"video_resolution_valid",passed:!expectedHeight||height===expectedHeight});
 ...
 const normalizedCues=cues.map((x:any)=>({start:Number(x.start),end:Number(x.end),text:String(x.text||"").trim()}));
